...
{	
	...
	while (dx < dy) {
		LCD_DrawDot(x+usC, y+usP, BLACK);
		LCD_DrawDot((-1*x)+usC, y+usP, BLACK);
		LCD_DrawDot(x+usC, (-1*y)+usP, BLACK);
		LCD_DrawDot((-1*x)+usC, (-1*y)+usP, BLACK);
...
void LCD_DrawLine_Dir(uint16_t initC, uint16_t initP, int length, int dir) {
	uint16_t currentC = initC;
	uint16_t currentP = initP;
	int currentLength = 0;
	
	while (currentLength <= length) {
		LCD_DrawDot(currentC, currentP, BLACK);
		currentLength++;
		currentP++;
		if (dir == 0) // left
			currentC--;
